...
**v1.1.0** is the current release, building on **v1.0.0** (first **production-ready** release candidate) and **v0.10.x** daily monitoring stabilisation.

**v1.1.0** adds **admin user management** (admin/operator roles, admin-only **Users** page with create/edit/delete, password reset, last sign-in tracking, daily summary subscription management, and admin safety protections) and **profile account controls** (name and password updates; login email is read-only).
...
- **Users and roles**: **admin** and **operator** roles. Admins see an admin-only **Users** page (`/admin/users`) to create, edit, and delete accounts, reset passwords, and manage daily summary subscriptions; the list shows name, login email, role, subscription, and last sign-in. Login email is set at creation and is read-only in the UI. Operators use the monitoring UI only. Bootstrap the first account with `php artisan contextual-console:create-admin-user`; use `contextual-console:promote-user-to-admin` to promote an existing user. Do not commit or document real passwords.
