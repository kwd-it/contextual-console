...
**v1.0.0** is the first **production-ready** release candidate, building on **v0.10.x** daily monitoring stabilisation. Earlier releases introduced deployment-ready monitoring (**v0.4.0**), scheduler timezone defaults and broader plot field tracking (**v0.5.0**), optional SQLite backups (**v0.6.x**), dashboard and cross-source Issues/Changes pages (**v0.7.0**), display metadata and UI polish (**v0.8.x**), issue review workflow and pagination (**v0.9.0**), and improved daily summary email plus per-user Profile opt-in (**v0.10.0**). **v1.0.0** adds issue detail pages with suggested checks and links from operator views, source health overview, manual **Run now**, failed-run diagnostics, daily summary email links and subscription warnings, Profile **Send test email**, UI timestamps in the schedule timezone, production operations documentation (`docs/OPERATIONS.md`), and deploy guidance to skip `config:cache` while HTTP source auth uses runtime `env()`. Ingest, comparison, change detection, and plot dataset issue detection rules are unchanged. See `CHANGELOG.md` and `docs/contextual-console-handover.md` for operator detail.
