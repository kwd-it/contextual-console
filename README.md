...
**v1.2.0** is the current release, building on **v1.1.1**, **v1.1.0**, **v1.0.0** (first **production-ready** release candidate), and **v0.10.x** daily monitoring stabilisation.

**v1.2.0** adds **Housebuilder Pack asset completeness** support: Console preserves the new plot payload fields, normalises completeness string values during HTTP ingest, and raises conservative warnings when upstream data reports a **missing** required floor plan or **missing** linked development intro media. It treats `unknown` completeness statuses as non-actionable and does not warn just because `has_floor_plan`, `has_intro_video`, or `has_intro_image` is false. Asset completeness fields are not plot change-detection fields; **`last_modified_by`** remains display-only metadata. Live payloads need **Housebuilder Pack v0.4.6 or later** for these fields; whether a monitored source produces live missing-asset warnings still depends on upstream mapping/rules and real data.
