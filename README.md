...
- **Scheduled monitoring** (Laravel scheduler; times use `config/app.php` **schedule timezone**, default **Europe/London**): `contextual-console:run-scheduled-sources` daily at **06:00**; **daily summary email** via `contextual-console:daily-summary --email` at **06:30** (HTML plus plain text; subscribed users at login email, otherwise `CONTEXTUAL_CONSOLE_DAILY_SUMMARY_TO`; mail env in `docs/DEPLOYMENT.md`). Local/dev **email preview** at `/dev/daily-summary-email-preview` when not in production.
