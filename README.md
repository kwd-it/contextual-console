...
**v1.2.1** is the current release, building on **v1.2.0**, **v1.1.1**, **v1.1.0**, **v1.0.0** (first **production-ready** release candidate), and **v0.10.x** daily monitoring stabilisation.

**v1.2.1** corrects how **development intro media** asset completeness is reported. Intro media describes the linked **development**, not the individual plot, and the source repeats it on every plot row, so the previous behaviour created one warning per plot assigned to the same development. Console no longer raises a plot-level issue for missing intro media. **Missing floor plan warnings remain plot-level**, and intro media fields are **still preserved** on stored snapshot payloads. Development-level completeness should be handled later through a dedicated **development ingest/snapshot/issue path**.

**v1.2.0** originally added **Housebuilder Pack asset completeness** support: Console preserves the new plot payload fields, normalises completeness string values during HTTP ingest, and raised conservative warnings when upstream data reported a **missing** required floor plan or a **missing** linked development intro media. **v1.2.1** later changed that behaviour so missing intro media is **no longer** raised as a plot-level warning (see above); the current behaviour is that a missing required floor plan still warns at plot level, missing intro media does not, and intro media fields are preserved on snapshots. v1.2.0 already treats `unknown` completeness statuses as non-actionable and does not warn just because `has_floor_plan`, `has_intro_video`, or `has_intro_image` is false. Asset completeness fields are not plot change-detection fields; **`last_modified_by`** remains display-only metadata. Live payloads need **Housebuilder Pack v0.4.6 or later** for these fields; whether a monitored source produces live missing-asset warnings still depends on upstream mapping/rules and real data.

**v1.1.1** completed the `/admin/users` UI actions introduced in **v1.1.0**: create user, admin password reset for other users, safe delete, and initial-password wording (invite emails and self-service password reset are not implemented). **v1.1.0** added **admin user management** (admin/operator roles, admin-only **Users** page, last sign-in tracking, daily summary subscription management, and admin safety protections) and **profile account controls** (name and password updates; login email is read-only).
...
See `CHANGELOG.md` and `docs/contextual-console-handover.md` for operator detail.
...
- **Issue detection**: dataset-level validation (invalid/missing ids, duplicates, missing/invalid `status`, status-aware `price` rules, and conservative **asset completeness** warnings when upstream payloads report actionable `missing` statuses). See `PlotDatasetIssueDetector`.
...
**Expected plot records** (after unwrap and optional adapter): a JSON array of objects. Each object should include a stable `id` for comparison. **`status`** must be one of `available`, `coming_soon`, `reserved`, or `sold` (case-insensitive). Missing **`status`** is warned. **`price`**: a missing or empty price triggers a warning **only** when `status` is `available`; for other valid statuses, omitting price is normal. If `price` is present and non-empty, it must be numeric and greater than or equal to 0 regardless of status. Optional **`last_modified_by`** (when returned by the Housebuilder Pack endpoint) is stored on snapshots and shown in the UI as **Last modified by: {label}**; it is display-only metadata and is not compared for change logs or issues. With **Housebuilder Pack v0.4.6 or later**, plot payloads may also include asset completeness fields (`has_floor_plan`, `floor_plan_required`, `floor_plan_completeness_status`, `has_intro_video`, `has_intro_image`, `intro_media_type`, `intro_media_completeness_status`). Console stores all of these on snapshots. As of **v1.2.1**, the only plot-level asset warning is a **required floor plan missing** (`floor_plan_required === true` and `floor_plan_completeness_status === "missing"`). Intro media describes the linked **development** rather than the plot, so missing intro media is **not** raised as a plot-level issue (the fields are still preserved on snapshots for a future development-level path). **`unknown`** completeness statuses are non-actionable. Console does not warn just because `has_floor_plan`, `has_intro_video`, or `has_intro_image` is false. Asset completeness fields are not plot change-detection fields. A **ContextualWP Housebuilder Pack** plots endpoint that already returns a **top-level array** of objects with these Console-aligned fields needs no `http_json_items_key` or `http_plot_payload_adapter`. See `PlotDatasetIssueDetector`.
